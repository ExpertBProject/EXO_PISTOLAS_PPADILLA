CREATE PROCEDURE EXO_GP_TRABAJO_LISTA_PICKING
(
...
BEGIN
  ...
  --Los de lote asignados  
	...
	SELECT T0."AbsEntry" AbsEntry, T1."PickEntry" LinPicket, T2."ItemCode" ItemCode, T2."Dscription" ItemName,T1."PickQtty" PickQtty, T4."Quantity" Cantidad, 
			...
			EXO_GP_GESTIONAR_EN_PICK(T0."AbsEntry", t1."PickEntry", T4."Quantity", T4."BatchNum") SePuedeGestionar,COALESCE(T3."NumInSale",1) NumPerMsr,COALESCE(T3."U_EXO_API",9999) Apilable
 	FROM "OPKL" T0	   
	...
	WHERE  T0."AbsEntry" = :ClavePicking and COALESCE(T4."Quantity", 0) != 0;
	...
	SELECT T2."AbsEntry" AbsEntry, T2."PickEntry" LinPicket,
	   		...
	   		'' BinCode,'N'  propuesto, 'Y' EsLote, 'Y' SePuedeGestionar ,COALESCE(TArt."NumInSale",1) NumPerMsr,COALESCE(TArt."U_EXO_API",9999) Apilable
	FROM "OWTR" T0
	...
	DECLARE CURSOR c_2 FOR
 	SELECT T2."AbsEntry" AbsEntry, T2."PickEntry" LinPicket,
			...
			'N' propuesto, 'N' EsLote, 'Y' SePuedeGestionar  ,COALESCE(TArt."NumInSale",1) NumPerMsr ,COALESCE(TArt."U_EXO_API",9999) Apilable
	FROM "OWTR" T0 
		INNER JOIN "WTR1" T1 ON T0."DocEntry" = T1."DocEntry"
		INNER JOIN "OITM" TArt on TArt."ItemCode" = T1."ItemCode"
		INNER JOIN "PKL1" T2 ON T2."AbsEntry" = T0."U_EXO_NUMPIC" AND T2."PickEntry" = T0."U_EXO_LINPIC"									
	...
	SELECT T1."AbsEntry" AbsEntry, T1."PickEntry" PickEntry, T2."ItemCode" ItemCode, T2."Dscription" ItemName, 
			...
			T2."WhsCode" Almacen, T3."ManBtchNum" ConLote,COALESCE(T3."NumInSale",1) NumPerMsr,coalesce(T3."U_EXO_API",9999) Apilable
	FROM "OPKL" T0
		INNER JOIN "PKL1" T1 ON T0."AbsEntry" = T1."AbsEntry"
		INNER JOIN "RDR1" T2 ON T1."BaseObject" = T2."ObjType" AND T2."DocEntry" = T1."OrderEntry" AND T2."LineNum" = T1."OrderLine"
		INNER JOIN "OITM" T3 ON T3."ItemCode" = T2."ItemCode" 
	...
	FOR c_row_0 AS c_0 DO
		...
		c_row_0.Udm,c_row_0.BatchNum,c_row_0.BinCode,c_row_0.propuesto,c_row_0.EsLote,c_row_0.SePuedeGestionar,c_row_0.NumPerMsr,c_row_0.Apilable);
	END FOR;	
	...
	FOR c_row_1 AS c_1 DO
		...
		c_row_1.Udm,c_row_1.BatchNum,c_row_1.BinCode,c_row_1.propuesto,c_row_1.EsLote,c_row_1.SePuedeGestionar,c_row_1.NumPerMsr,c_row_1.Apilable);
	END FOR;
	...
	FOR c_row_2 AS c_2 DO
		...
		c_row_2.Udm,c_row_2.BatchNum,c_row_2.BinCode,c_row_2.propuesto,c_row_2.EsLote,c_row_2.SePuedeGestionar,c_row_2.NumPerMsr,c_row_2.Apilable);
	END FOR;
	...
	FOR c_row_3 AS cPicking_3 DO
 		...
 		IF COALESCE(vCantidadAsignada,0) < c_row_3.CantLinea THEN
			...
			c_row_3.Udm,vLotePropuesto,vUbicProp,'Y',c_row_3.ConLote,'N',c_row_3.NumPerMsr,c_row_3.Apilable);
	...
	TablaDatos = SELECT AbsEntry,PickEntry,ItemCode,ItemName,MAX(CantidadTotal) CantidadTotal,SUM(Cantidad) Cantidad,Udm,BatchNum,BinCode,Propuesto,EsLote, SePuedeGestionar,NumPerMsr  
	FROM #tmp 
	GROUP BY AbsEntry,PickEntry,ItemCode,ItemName,Udm,BatchNum,BinCode,Propuesto,EsLote, SePuedeGestionar,NumPerMsr,Apilable
	ORDER BY Apilable;
